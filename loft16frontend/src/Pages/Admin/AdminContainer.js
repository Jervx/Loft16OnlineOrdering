...
const AdminContainer = (props) => {
  ...
  const amISignedIn = async () => {
    try {
      ...
      const response = await API.get(`/admin/mydetails/${savedAdmin._id}`);
    } catch (error) {
      if (error.response) {
  ...
  return (
    <div onClick={(e)=>{
            ...
            <ul key={idx} className="flex flex-col py-2 mb-3">
              ...
              {routeGroup.routes.map((routes, idx) => (
                <li key={idx}>
                  <Link
                    to={routes.path}
                    className={`filter transition overflow-x-hidden duration-200 ease-in-out motion-reduce:transition-none motion-reduce:transform-none relative flex flex-row items-center h-11 focus:outline-none hover:bg-gray-50 text-teal-500 border-l-4 border-transparent hover:border-teal-700 pr-6 hover:text-teal-700 ${
                      location.pathname === routes.path &&
                      "bg-gray-50 italic shadow-inner rounded-lg border-teal-500 font-medium text-teal-600"
                    } `}
                  >
      ...
      <aside className={`${toggleSide? 'left-0 z-10' : '-left-80'} duration-500 absolute md:hidden z-30 flex-shrink-0 px-4 w-64 overflow-y-auto bg-white dark:bg-gray-800`}>
        <div className="flex items-center my-6">
          <Link
            className=" MoonTime defTextCOlorGreen lg:block ml-6 text-2xl font-bold text-gray-800 dark:text-gray-200"
            to="/admin"
