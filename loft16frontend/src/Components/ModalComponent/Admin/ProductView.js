...
const ProductView = ({ _id, data, mode, onSave }) => {
  ...
  const loadSomething = async () => {
    try {
      ...
      if (mode !== 0) {
        const prod = await API.get(`/admin/getproductdetail/${data._id}`);
  ...
  const saveImage = async (data) => {
    try {
      ...
      fdata.append("_id", _id);
      fdata.append("prod_Id", data._id);

      for (let x = 0; x < fileImages.length; x++)
        fdata.append("fileImages", fileImages[x]);
            ...
            <div className="mt-6">
              ...
              {newImages.length !== 0 && (
                <HelperLabel
                  ...
                  bg={"bg-teal-50"}
                  txtbg={"text-teal-600"}
                  msg={"The photo you added will be uploaded after saving"}
                />
              )}
            </div>
          </>
